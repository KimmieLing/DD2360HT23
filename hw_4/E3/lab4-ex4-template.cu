...
int main(int argc, char **argv) {
  ...
  gpuCheck(cudaMallocManaged(&temp, dimX*sizeof(double))); //Unsure
  gpuCheck(cudaMallocManaged(&A, dimX * dimX *sizeof(double)));
  ...
  if (concurrentAccessQ) {
    ...
    gpuCheck(cudaMemPrefetchAsync(A, dimX * dimX * sizeof(double), cudaCpuDeviceId));
    gpuCheck(cudaMemPrefetchAsync(ARowPtr,(dimX + 1) * sizeof(int), cudaCpuDeviceId));
    gpuCheck(cudaMemPrefetchAsync(AColIndx, dimX * sizeof(int), cudaCpuDeviceId));
  ...
  if (concurrentAccessQ) {
    ...
    gpuCheck(cudaMemPrefetchAsync(tmp, dimX * sizeof(double), 0));
    gpuCheck(cudaMemPrefetchAsync(temp, dimX * sizeof(double), 0));
    gpuCheck(cudaMemPrefetchAsync(A, dimX * dimX * sizeof(double), 0));
    gpuCheck(cudaMemPrefetchAsync(ARowPtr, (dimX + 1) * sizeof(int), 0));
    gpuCheck(cudaMemPrefetchAsync(AColIndx, dimX * sizeof(int), 0));
  ...
  cusparseDnVecDescr_t cuSparseTemp;
  cusparseDnVecDescr_t cuSparseTmp;
  cusparseCheck(cusparseCreateDnVec(&cuSparseTemp, dimX, temp, CUDA_R_64F));
  cudaDeviceSynchronize();
  cusparseCheck(cusparseCreateDnVec(&cuSparseTmp, dimX, tmp, CUDA_R_64F));
  cudaDeviceSynchronize();

  //Create sparse matrix
  cusparseSpMatDescr_t cuSparseA;
  cusparseCheck(cusparseCreateCsr(&cuSparseA, dimX, dimX, nzv, ARowPtr, AColIndx, A, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ZERO, CUDA_R_64F));
  cudaDeviceSynchronize();

  //Get bufferSize
  cusparseCheck(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, cuSparseA, cuSparseTemp, &zero, cuSparseTmp, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize));
  cudaDeviceSynchronize();
  // Check if bufferSize is non-zero
if (bufferSize == 0) {
...
if (buffer == nullptr) {
  ...
  for (int it = 0; it < nsteps; ++it) {
    ...
    //@@ tmp = 1 * A * temp + 0 * tmp
    cusparseCheck(cusparseSpMV(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, cuSparseA, cuSparseTemp, &zero, cuSparseTmp, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, buffer));
    cudaDeviceSynchronize();
    ...
    //KANSKE FÖRA ÖVER CUSPARSETEMP OCH TMP TILL TEMP OCH TMP
  ...
  cusparseDestroyDnVec(cuSparseTemp);
  cusparseDestroyDnVec(cuSparseTmp);
  cusparseDestroySpMat(cuSparseA);
