...
int mover_PC(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param)
{
    ...
    // start subcycling WHAT IS part-n_sub_cycles?
...
__global__ void mover_PC_gpu(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param)
{
...
void mover_PC_launch_gpu(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param)
{
    particles *d_part;
    EMfield *d_field;
    grid *d_grid;
    ...
    mover_PC_gpu<<<gridSize,blockSize>>>(d_part, d_field, d_grid, d_param);
    cudaDeviceSynchronize();

    //only changes particle
    cudaMemcpy(part, d_part, sizeof(particles)*param->ns, cudaMemcpyDeviceToHost);
